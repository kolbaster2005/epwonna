...
export default function AdminTopics({ examKey }) {
  ...
  async function handleRename(topic, label) {
    ...
    try {
      ...
      window.alert(err.message || 'Не удалось переименовать тему.')
    }
  }

  async function handleDelete(topic) {
    if (!window.confirm(`Удалить тему «${topic.label}»? Пробники, у которых она уже выбрана, не удалятся, но тема пропадёт из выпадающего списка.`)) return
    try {
      await deleteTopic(examKey, topic.id)
      reload()
    } catch (err) {
      window.alert(err.message || 'Не удалось удалить тему.')
    }
  }
  ...
  async function move(topic, direction) {
    ...
    try {
      ...
      window.alert(err.message || 'Не удалось изменить порядок.')
    }
  }
